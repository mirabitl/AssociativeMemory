...
#include <sstream>
HoughLocal1::HoughLocal1(double thmin,double thmax,double rmin,double rmax,uint32_t nbintheta,uint32_t nbinr) : theThetaMin_(thmin),theThetaMax_(thmax),theRMin_(rmin),theRMax_(rmax),theNbinTheta_(nbintheta),theNbinR_(nbinr)
{
...
void HoughLocal1::initialise(double thmin,double thmax,double rmin,double rmax,uint32_t nbintheta,uint32_t nbinr)
{
...
HoughLocal1::~HoughLocal1(){;}
uint32_t HoughLocal1::getVoteMax(){return theVoteMax_;}
void HoughLocal1::addStub(stub_t s)
{
  //theVoteMax_=0;
  double z=s.x;
  double x=s.y;
...
void HoughLocal1::fill(double z, double x)
{
...
double HoughLocal1::getTheta(int32_t i) {return theThetaMin_+(i+0.5)*theThetaBin_;}
double HoughLocal1::getR(int32_t i) {return theRMin_+(i+0.5)*theRBin_;}
uint16_t HoughLocal1::getValue(uint32_t i,uint32_t j){return theHoughImage_[i][j];}
void HoughLocal1::findMaxima(std::vector< std::pair<uint32_t,uint32_t> >& maxval,uint32_t cut)
{
...
void HoughLocal1::findMaximumBins(std::vector< std::pair<double,double> >& maxval,uint32_t cut,std::vector<uint32_t> *ids)
{
	...
	if (matlap[ith][ir]==-8 && matrix[ith][ir]!=0) //isolated maximum
	  {
		...
		for (uint32_t it=0;it<theHoughMap_[ith][ir].size();it++)
		  if (std::find(ids->begin(),ids->end(), theHoughMap_[ith][ir][it])==ids->end()) ids->push_back(theHoughMap_[ith][ir][it]);
	      }
	  }
      }
...
void HoughLocal1::findThresholdBins(std::vector< std::pair<double,double> >& maxval,uint32_t cut)
{
...
void HoughLocal1::findMaxima(std::vector< std::pair<double,double> >& maxval,uint32_t cut)
{
...
void HoughLocal1::draw(DCHistogramHandler* h,std::vector< std::pair<uint32_t,uint32_t> > *maxval)
{
  if (CanvasHough==NULL)
    {
  ...
  htname<<"HoughLocal1"<<theNbinTheta_<<"_"<<theNbinR_;
...
void HoughLocal1::draw(DCHistogramHandler* h,std::vector< std::pair<double,double> > *maxval)
{
  if (CanvasHough==NULL)
    {
      CanvasHough=new TCanvas("CanvasHough","hough",800,900);
...
void HoughLocal1::PrintConvert(double theta,double r,mctrack_t *m)
{
  double a=-1./tan(theta);
  double b=r/sin(theta);
...
void HoughLocal1::Convert(double theta,double r,mctrack_t *m)
{
  double a=-1./tan(theta);
  double b=r/sin(theta);
